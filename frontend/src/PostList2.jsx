...
export default class PostList2 extends Component {
  ...
  editPost = async (id, content) => {
    ...
    const resp = await fetch(`/post2/${id}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  deletePost = async id => {
    const resp = await fetch(`/post2/${id}`, { method: 'DELETE' });
    const text = await resp.text();
    if (text !== 'DELETED') {
      throw new Error(`Bad response: ${text}`);
    }
